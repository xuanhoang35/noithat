            ...
            <?php foreach ($featured as $p): ?>
                ...
                <div class="floating fade-border rounded-2xl shadow-sm transition p-3 flex flex-col bg-white/90">
                    ...
                    <div class="mt-auto grid grid-cols-2 gap-2 items-stretch pt-1">
                        <?php $btnHeight = $stock <= 0 ? 'h-12' : 'h-11'; ?>
                        <a class="w-full text-center <?php echo $btnHeight; ?> flex items-center justify-center text-sm border rounded-lg border-slate-200 hover:border-blue-500" href="<?php echo base_url('product/' . $p['id']); ?>">Xem</a>
                        <?php if ($stock <= 0): ?>
