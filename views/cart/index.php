...
<div class="space-y-4">
    <div class="flex items-center justify-between">
        <div>
            <p class="text-sm text-slate-500">Giỏ hàng của bạn</p>
    ...
    <div class="grid lg:grid-cols-3 gap-4">
            ...
            <table class="min-w-full text-sm">
                ...
                <?php foreach($cartRows as $row): $item = $row['item']; $product = $item['product']; $available = (int)($product['stock'] ?? 0); $warn = $stockWarnings[$row['id']] ?? null; ?>
                    <tr class="border-b hover:bg-slate-50">
                        ...
                        <td class="p-3 text-blue-600"><?php echo number_format($product['price']); ?> đ</td>
                        <td class="p-3">
        ...
        <div class="bg-white rounded-2xl shadow-sm p-4 lg:sticky lg:top-24 h-fit">
            ...
            <form method="post" action="<?php echo base_url('cart/apply-voucher'); ?>" class="flex gap-2 mb-3">
                <input class="px-3 py-2 border rounded w-full" name="voucher_code" placeholder="Mã giảm giá" value="<?php echo htmlspecialchars($appliedVoucher['code'] ?? $prefillVoucher); ?>">
                <button class="px-4 py-2 bg-slate-900 text-white rounded hover:bg-slate-800">Áp dụng</button>
            </form>
            <?php if (!empty($appliedVoucher)): ?>
            ...
            <?php else: ?>
                <form method="post" action="<?php echo base_url('order/checkout'); ?>" class="grid grid-cols-1 gap-3">
                    ...
                    <textarea class="px-3 py-2 border rounded w-full" name="note" placeholder="Ghi chú"></textarea>
