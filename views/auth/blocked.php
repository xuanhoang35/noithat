...
<div class="max-w-md mx-auto text-center space-y-4">
    <div class="bg-red-50 text-red-700 border border-red-100 rounded-xl p-5 shadow-sm">
        ...
        <p class="text-sm"><?php echo htmlspecialchars($message ?? 'Phiên đăng nhập đã hết hạn.'); ?></p>
        <p class="text-xs text-red-500 mt-1">Bạn sẽ được đăng xuất trong giây lát.</p>
    </div>
    <div class="text-sm text-slate-500" id="countdown">Đăng xuất sau 5 giây...</div>
</div>
<script>
setTimeout(function(){ window.location.href = '<?php echo base_url('logout'); ?>'; }, 5000);
</script>
